...
#include "lat_long_functions.hpp"
...
{
    // check auth of the buyer
    require_auth(owner);

    // get the size of the land after verification of integrity
    std::pair<double, double> land_size = assert_lat_long_values(lat_north_edge, long_east_edge,
                                                                 lat_south_edge, long_west_edge);

    // table instance to save land
    land_table lands(_self, _self.value);

    // get minimum latitude greater than the southern edge
    ...
    asset inf_amount = asset(0, inf_symbol); // amount to pay for the land

    // start from southern edge and move upwards
    while (lands_itr != lat_north_index.end() && lands_itr->lat_north_edge < upper_bound)
    {
            ...
            l.long_west_edge >= long_west_edge)
        {
            ...
            // calculate cost of land already registered based on price and time, then deduct from fees
...
// check the conssistency of latitude and longitude values
                                                              ...
                                                              const double &long_east_edge, const double &lat_south_edge, const double &long_west_edge)
{
    ...
    eosio_assert(long_east_edge <= 180 && long_east_edge > -180 && long_west_edge <= 180 && long_west_edge > -180, "Longitude must be between -180 and 180 degrees");
